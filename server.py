...
@app.route('/charge-single', methods=['POST', 'OPTIONS'])
def charge_single_customer():
    ...
    try:
        ...
        try:
            ...
            # Get card details
            payment_method = stripe.PaymentMethod.retrieve(customer['payment_method_id'])
            card_info = {
                'brand': payment_method.card.brand if hasattr(payment_method, 'card') else 'N/A',
                'last4': payment_method.card.last4 if hasattr(payment_method, 'card') else 'N/A',
                'exp_month': payment_method.card.exp_month if hasattr(payment_method, 'card') else 'N/A',
                'exp_year': payment_method.card.exp_year if hasattr(payment_method, 'card') else 'N/A'
            }
        ...
        print(f"❌ Error in charge_single_customer: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 400
